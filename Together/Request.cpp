...
/*   Updated: 2024/07/04 12:07:06 by jp-de-to         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "Request.hpp"

Request::Request( char str[], size_t size, Server& configServer) : _request( str, size ), _configServer( configServer ), _code( 0 )
{
	std::cout << _code << '\n';
}
...
std::string	Request::buildResponse()
{
	...
	parseRequest( requestLine, body );
	// check _code.
	return "hello world";
}

void	Request::parseRequest( std::string& requestLine, std::string& body )
{
	std::istringstream	request( _request );
	if (std::getline( request, requestLine ))
	{
		if (isGoodRequestLine( requestLine ) == false)
			return;
	}
	...
	if (isGoodBody( body ) == false)
		return;
}

int	Request::isGoodRequestLine( std::string& requestLine )
{
	if (requestLine.find( "\r\n", 0 ) == std::string::npos)
	{
		_code = 400;
...
int	Request::isGoodBody( std::string& body )
{
